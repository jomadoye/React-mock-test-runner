...
describe('App.js page structure', () => {
  test('should render app title', () => {
    ...
    expect(getByText("Jed's Automated Test Runner V1")).toHaveTextContent("Jed's Automated Test Runner V1")
  })
  ...
  test('should render "Amount of tests that have passed so far"', () => {
    ...
    expect(getByText("Amount of tests that have passed so far:")).toHaveTextContent("Amount of tests that have passed so far:")
  })

  test('should render "Amount of tests that have failed so far"', () => {
    const { getByText, } = render(<App />)

    expect(getByText("Amount of tests that have failed so far:")).toHaveTextContent("Amount of tests that have failed so far:")
  })

  test('should render "Amount of tests that are still running"', () => {
    const { getByText } = render(<App />)

    expect(getByText("Amount of tests that are still running:")).toHaveTextContent("Amount of tests that are still running:")
  })

  test('should render "All test completed"', () => {
    const { getByText } = render(<App />)

    expect(getByText("All test completed:")).toHaveTextContent("All test completed:")
  })
  ...
  test('should render child table with Description column', () => {
    ...
    expect(getByText("uploads go in both directions")).toHaveTextContent("uploads go in both directions")
  })
